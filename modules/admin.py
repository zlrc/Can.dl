...
async def cherrypick(ctx, keyword, amount=10):
    ...
    if ctx.message.author.server_permissions.manage_messages: # Person using this command can delete messages

        # Break up message into a list of individual characters.
        char = [str(s) for s in keyword]

        if ('@' and '<' and '>') in char: # Detects if a mention is used
            await bot.send_typing(ctx.message.channel)

            # Deletes messages where author id matches mentioned user's id, but isn't the original message.
            def usr(m):
            ...
            # Deletes messages where the keyword is found
            def key(m):
